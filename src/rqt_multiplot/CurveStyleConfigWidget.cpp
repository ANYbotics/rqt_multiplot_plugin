...
CurveStyleConfigWidget::CurveStyleConfigWidget(QWidget* parent)
  ...
  ui_->lineEditSticksBaseline->setValidator(new QDoubleValidator(ui_->lineEditSticksBaseline));
  ...
  buttonGroupSticksOrientation_->addButton(ui_->radioButtonSticksOrientationHorizontal);
  buttonGroupSticksOrientation_->addButton(ui_->radioButtonSticksOrientationVertical);

  connect(ui_->radioButtonLines, SIGNAL(toggled(bool)), this, SLOT(radioButtonLinesToggled(bool)));
  connect(ui_->radioButtonSticks, SIGNAL(toggled(bool)), this, SLOT(radioButtonSticksToggled(bool)));
  connect(ui_->radioButtonSteps, SIGNAL(toggled(bool)), this, SLOT(radioButtonStepsToggled(bool)));
  connect(ui_->radioButtonPoints, SIGNAL(toggled(bool)), this, SLOT(radioButtonPointsToggled(bool)));

  connect(ui_->checkBoxLinesInterpolate, SIGNAL(stateChanged(int)), this, SLOT(checkBoxLinesInterpolateStateChanged(int)));
  connect(ui_->radioButtonSticksOrientationHorizontal, SIGNAL(toggled(bool)), this,
          SLOT(radioButtonSticksOrientationHorizontalToggled(bool)));
  connect(ui_->radioButtonSticksOrientationVertical, SIGNAL(toggled(bool)), this, SLOT(radioButtonSticksOrientationVerticalToggled(bool)));
  connect(ui_->lineEditSticksBaseline, SIGNAL(editingFinished()), this, SLOT(lineEditSticksBaselineEditingFinished()));
  connect(ui_->checkBoxStepsInvert, SIGNAL(stateChanged(int)), this, SLOT(checkBoxStepsInvertStateChanged(int)));
...
void CurveStyleConfigWidget::setConfig(CurveStyleConfig* config) {
  if (config != config_) {
    if (config_ != nullptr) {
      disconnect(config_, SIGNAL(typeChanged(int)), this, SLOT(styleTypeChanged(int)));

      disconnect(config_, SIGNAL(linesInterpolateChanged(bool)), this, SLOT(configLinesInterpolateChanged(bool)));
      disconnect(config_, SIGNAL(sticksOrientationChanged(int)), this, SLOT(configSticksOrientationChanged(int)));
      disconnect(config_, SIGNAL(sticksBaselineChanged(double)), this, SLOT(configSticksBaselineChanged(double)));
      disconnect(config_, SIGNAL(stepsInvertChanged(bool)), this, SLOT(configStepsInvertChanged(bool)));
    ...
    if (config != nullptr) {
      ...
      connect(config, SIGNAL(linesInterpolateChanged(bool)), this, SLOT(configLinesInterpolateChanged(bool)));
      connect(config, SIGNAL(sticksOrientationChanged(int)), this, SLOT(configSticksOrientationChanged(int)));
      connect(config, SIGNAL(sticksBaselineChanged(double)), this, SLOT(configSticksBaselineChanged(double)));
      connect(config, SIGNAL(stepsInvertChanged(bool)), this, SLOT(configStepsInvertChanged(bool)));
...
void CurveStyleConfigWidget::configLinesInterpolateChanged(bool interpolate) {
  ui_->checkBoxLinesInterpolate->setCheckState(interpolate ? Qt::Checked : Qt::Unchecked);
}

void CurveStyleConfigWidget::configSticksOrientationChanged(int orientation) {
  ui_->radioButtonSticksOrientationHorizontal->setChecked(orientation == Qt::Horizontal);
  ui_->radioButtonSticksOrientationVertical->setChecked(orientation == Qt::Vertical);
}
...
void CurveStyleConfigWidget::configStepsInvertChanged(bool invert) {
  ui_->checkBoxStepsInvert->setCheckState(invert ? Qt::Checked : Qt::Unchecked);
}
...
void CurveStyleConfigWidget::configRenderAntialiasChanged(bool antialias) {
  ui_->checkBoxRenderAntialias->setCheckState(antialias ? Qt::Checked : Qt::Unchecked);
}
...
void CurveStyleConfigWidget::radioButtonSticksOrientationHorizontalToggled(bool checked) {
  if ((config_ != nullptr) && checked) {
    config_->setSticksOrientation(Qt::Horizontal);
  }
}

void CurveStyleConfigWidget::radioButtonSticksOrientationVerticalToggled(bool checked) {
  if ((config_ != nullptr) && checked) {
...
void CurveStyleConfigWidget::lineEditSticksBaselineEditingFinished() {
  if (config_ != nullptr) {
    config_->setSticksBaseline(ui_->lineEditSticksBaseline->text().toDouble());
  }
}
