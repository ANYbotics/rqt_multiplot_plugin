...
CurveAxisScaleConfigWidget::CurveAxisScaleConfigWidget(QWidget* parent)
  ...
  ui_->lineEditAbsoluteMinimum->setValidator(new QDoubleValidator(ui_->lineEditAbsoluteMinimum));
  ui_->lineEditAbsoluteMaximum->setValidator(new QDoubleValidator(ui_->lineEditAbsoluteMaximum));
  ui_->lineEditRelativeMinimum->setValidator(new QDoubleValidator(ui_->lineEditRelativeMinimum));
  ui_->lineEditRelativeMaximum->setValidator(new QDoubleValidator(ui_->lineEditRelativeMaximum));

  connect(ui_->radioButtonAbsolute, SIGNAL(toggled(bool)), this, SLOT(radioButtonAbsoluteToggled(bool)));
  connect(ui_->radioButtonRelative, SIGNAL(toggled(bool)), this, SLOT(radioButtonRelativeToggled(bool)));
  connect(ui_->radioButtonAuto, SIGNAL(toggled(bool)), this, SLOT(radioButtonAutoToggled(bool)));

  connect(ui_->lineEditAbsoluteMinimum, SIGNAL(editingFinished()), this, SLOT(lineEditAbsoluteMinimumEditingFinished()));
  connect(ui_->lineEditAbsoluteMaximum, SIGNAL(editingFinished()), this, SLOT(lineEditAbsoluteMaximumEditingFinished()));
  connect(ui_->lineEditRelativeMinimum, SIGNAL(editingFinished()), this, SLOT(lineEditRelativeMinimumEditingFinished()));
  connect(ui_->lineEditRelativeMaximum, SIGNAL(editingFinished()), this, SLOT(lineEditRelativeMaximumEditingFinished()));
}
...
void CurveAxisScaleConfigWidget::setConfig(CurveAxisScaleConfig* config) {
  if (config != config_) {
    if (config_ != nullptr) {
      ...
      disconnect(config_, SIGNAL(absoluteMinimumChanged(double)), this, SLOT(configAbsoluteMinimumChanged(double)));
      disconnect(config_, SIGNAL(absoluteMaximumChanged(double)), this, SLOT(configAbsoluteMaximumChanged(double)));
      disconnect(config_, SIGNAL(relativeMinimumChanged(double)), this, SLOT(configRelativeMinimumChanged(double)));
      disconnect(config_, SIGNAL(relativeMaximumChanged(double)), this, SLOT(configRelativeMaximumChanged(double)));
    }
    ...
    if (config != nullptr) {
      ...
      connect(config, SIGNAL(absoluteMinimumChanged(double)), this, SLOT(configAbsoluteMinimumChanged(double)));
      connect(config, SIGNAL(absoluteMaximumChanged(double)), this, SLOT(configAbsoluteMaximumChanged(double)));
      connect(config, SIGNAL(relativeMinimumChanged(double)), this, SLOT(configRelativeMinimumChanged(double)));
      connect(config, SIGNAL(relativeMaximumChanged(double)), this, SLOT(configRelativeMaximumChanged(double)));
...
void CurveAxisScaleConfigWidget::lineEditAbsoluteMinimumEditingFinished() {
  if (config_ != nullptr) {
    config_->setAbsoluteMinimum(ui_->lineEditAbsoluteMinimum->text().toDouble());
  }
}

void CurveAxisScaleConfigWidget::lineEditAbsoluteMaximumEditingFinished() {
  if (config_ != nullptr) {
    config_->setAbsoluteMaximum(ui_->lineEditAbsoluteMaximum->text().toDouble());
  }
}

void CurveAxisScaleConfigWidget::lineEditRelativeMinimumEditingFinished() {
  if (config_ != nullptr) {
    config_->setRelativeMinimum(ui_->lineEditRelativeMinimum->text().toDouble());
  }
}

void CurveAxisScaleConfigWidget::lineEditRelativeMaximumEditingFinished() {
  if (config_ != nullptr) {
    config_->setRelativeMaximum(ui_->lineEditRelativeMaximum->text().toDouble());
  }
}
