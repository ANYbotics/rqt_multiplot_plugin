...
QVector<size_t> CurveDataCircularBuffer::getPointsInDistance(double x, double maxDistance) const {
  QVector<size_t> indexes;

  XCoordinateRefMinHeap::ordered_iterator it = std::lower_bound(xMin_.ordered_begin(), xMin_.ordered_end(), x - maxDistance);

  while (it != xMin_.ordered_end()) {
    if (fabs(x - it->x_) <= maxDistance) {
      size_t index = it->index_;

      if (points_.array_two().second != 0u) {
        index = (index < points_.array_two().second) ? index + points_.array_one().second : index - points_.array_two().second;
      }
...
void CurveDataCircularBuffer::appendPoint(const QPointF& point) {
  ...
  if (points_.array_two().second != 0u) {
    index = (points_.array_one().first < points_.array_two().first) ? &points_.back() - points_.array_one().first
